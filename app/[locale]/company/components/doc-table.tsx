...
const INITIAL_VISIBLE_COLUMNS = ["Name", "Size", "Date", "Actions"];

export default function DocTable({ documents }: { documents: Document[] }) {
  ...
  const sortedItems = React.useMemo(() => {
    return [...items].sort((a, b) => {
      const first = a[sortDescriptor.column as keyof Document];
      const second = b[sortDescriptor.column as keyof Document];
  ...
  const renderCell = React.useCallback(
    (document: Document, columnKey: string) => {
      const cellValue = document[columnKey as keyof Document];
      switch (columnKey) {
        case "Name":
          return (
            <p className="text-bold text-small capitalize">{document.Name}</p>
          );
        case "Size":
          return (
        ...
        case "Date":
          return (
            <div className="text-bold text-small capitalize">
              {new Date(document.Date).toLocaleDateString("uz-UZ", {
                day: "numeric",
                month: "numeric",
                year: "numeric",
              })}
