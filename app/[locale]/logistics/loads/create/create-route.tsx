...
import { AddressSelectorBox } from "@/components/forms/AddressSelector";
import {
  LoadReadyState,
  Workdays,
...
import { getGeoPlaces } from "@/types/api";
      ...
      <div className="flex items-start justify-start gap-3 flex-wrap w-full">
        ...
        {loadReadyState == LoadReadyState.LoadIsReadyAt && (
          <DateRangePicker
            ...
            value={
              loadRoute && loadRoute.when && loadRoute.when.dateInterval
                ? (() => {
                    const start = loadRoute.when.dateInterval.start
                      ? new CalendarDate(
                          loadRoute.when.dateInterval.start.getFullYear(),
                          loadRoute.when.dateInterval.start.getMonth() + 1,
                          loadRoute.when.dateInterval.start.getDate(),
                        )
                      : undefined;
                    const end = loadRoute.when.dateInterval.end
                      ? new CalendarDate(
                          loadRoute.when.dateInterval.end.getFullYear(),
                          loadRoute.when.dateInterval.end.getMonth() + 1,
                          loadRoute.when.dateInterval.end.getDate(),
                        )
                      : undefined;
            ...
            onChange={(val) => {
              ...
              const toJSDate = (dateValue: DateValue | null | undefined) =>
                dateValue ? dateValue.toDate(getLocalTimeZone()) : undefined;

              const dateInterval = val
                ? {
                    start: toJSDate(val.start),
                    end: toJSDate(val.end),
                  }
                : undefined;
              ...
              if (loadRoute) {
                updateLoadRoute({ ...loadRoute, when });
              } else {
                updateLoadRoute({ when });
              }
            }}
          />
        )}
      ...
      <div className="flex items-end justify-start gap-3 flex-wrap w-full">
        ...
        <div className="flex gap-3 flex-wrap">
          <Input
            className="w-80"
            variant="faded"
            description="Write address of the unloading place"
            endContent={
              <Button isIconOnly variant="light" className="-mr-2" size="sm">
                <Icon
                  icon="line-md:map-marker-plus"
        ...
        <DateRangePicker
          ...
          value={
            ...
            loadRoute.downloading.downloadingTime.dateRange
              ? (() => {
                  const start = loadRoute.downloading.downloadingTime.dateRange
                    .start
                    ? new CalendarDate(
                        loadRoute.downloading.downloadingTime.dateRange.start.getFullYear(),
                        loadRoute.downloading.downloadingTime.dateRange.start.getMonth() +
                          1,
                        loadRoute.downloading.downloadingTime.dateRange.start.getDate(),
                      )
                    : undefined;
                  const end = loadRoute.downloading.downloadingTime.dateRange
                    .end
                    ? new CalendarDate(
                        loadRoute.downloading.downloadingTime.dateRange.end.getFullYear(),
                        loadRoute.downloading.downloadingTime.dateRange.end.getMonth() +
                          1,
                        loadRoute.downloading.downloadingTime.dateRange.end.getDate(),
                      )
                    : undefined;
                  return start && end ? { start, end } : undefined;
                })()
